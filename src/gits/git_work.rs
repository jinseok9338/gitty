...
use crate::{arguments::{multiselect::MultiSelect, common_trait::{Default, Run}}, logs::loading::Loading};
...
impl GitWork {
    ...
    pub async fn gitty_clone_repo(&self, url: &str, directory: &PathBuf) -> Result<(), Box<dyn Error>> {
        ...
        for branch in selected_branches {
            let local_branch = self.git_helper.create_local_branch(&cloned_repo, &branch).unwrap();
            println!("local_branch: {:?}", local_branch.name().unwrap());

            let remote_branch = self.git_helper.remote_branch(&cloned_repo, &branch).unwrap();
            println!("remote_branch: {:?}", remote_branch.name().unwrap());

            let sync_branch = self.git_helper.sync_branch(&cloned_repo, &local_branch, &remote_branch).unwrap();
