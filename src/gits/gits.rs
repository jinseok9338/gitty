...
use git2::{Repository, Error, BranchType, Branches, RemoteHead, Remote};
...
impl GitWork {
  //for cloining the repository
    ...
    pub fn fetch_branch(&self, repo: &Repository, branch: &str) -> Result<(), Error> {
        let mut remote = repo.find_remote("origin")?;
        remote.fetch(&[&format!("refs/heads/{}:refs/heads/{}", branch, branch)], None, None)?;
        Ok(())
    }
    ...
    pub fn get_local_branches<'a>(&'a self, repo:&'a Repository, filter:Option<BranchType> 
