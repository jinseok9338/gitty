    ...
    ($cmd:expr) => {{
        ...
        pb.set_style(
            ProgressStyle::default_bar()
                .template("{spinner:.red} Running command...").expect("Unable to set template")
                .progress_chars("#>-"),
        );
   
        pb.set_position(0);
        let (tx, rx): (mpsc::Sender<std::process::Output>, mpsc::Receiver<std::process::Output>) = mpsc::channel();
